...
### Stake Token
ERC20-compliant token deposited by the user for staking

### Reward Token
ERC20-compliant token distributed as a staking reward
...
- Redirects rewards to the vault when no active staking exists to avoid loss of unallocated reward
...
- No emergency `withdraw` mechanism is implemented for security & transparency reasons
- Solidity version is locked to 0.5.7

## Disclaimer
This contract has not been audited yet. It is highly recommended that anyone planning to use it review it thoroughly before deploying it in production or elsewhere.
